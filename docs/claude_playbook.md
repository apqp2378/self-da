...
**왜 이 문서** · "퀄리티가 좋지 않다"의 원인은 대개 **일반적 프롬프트 → 일반적 출력**. 도구가 아니라 작업 설계가 품질을 만든다.
...
- **현재 작업:** Module A 토픽 라벨링·Bullet 추출 프롬프트에 **ICC+컨텍스트 인터뷰**를 붙인다. 특히 토픽 라벨은 "먼저 domain_taxonomy.md를 읽고 매핑을 검증한 뒤(ground first) 라벨 제안" 형태로.
- **반복 작업 → 스킬/스케줄 후보 (Rule of Rs 통과):**
  - JD 등록(프롬프트 #1) — 규칙적, 반복 → 커스텀 스킬 후보
  - Daily Log 기록(프롬프트 #2) — 매일, 규칙적 → **스케줄 태스크** 후보(매일 저녁 자동)
  - 주간 Bullet 추출(프롬프트 #3) — 주1회 → 스킬 후보
  - (이미 보유) youtube-script-to-learning-note 스킬 ✓
- **모델 선택:** 기본 Sonnet, 코드·설계 빌드 시 Opus, 50개 노트 요약 같은 대량 단순작업은 Haiku.
